...
public class Line_Text
{
    ...
    public Line_Text(int startVertexIndex,int length)
    {
...
public class TextSpacing : BaseMeshEffect
{
    ...
    public override void ModifyMesh(VertexHelper vh)
    {
        ...
        Line_Text[] lines = new Line_Text[lineTexts.Length];
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (i == 0)
            {
                lines[i] = new Line_Text(0, lineTexts[i].Length + 1);
            }
            else if(i > 0 && i < lines.Length - 1)
            {
                lines[i] = new Line_Text(lines[i - 1].EndVertexIndex + 1, lineTexts[i].Length + 1);
            }
            else
            {
                lines[i] = new Line_Text(lines[i - 1].EndVertexIndex + 1, lineTexts[i].Length);
            }
        }
